...
Model.embedsMany = function embedsMany(anotherClass, params) {
    ...
    function all(params, cb) {
        ...
        var coll = _.map(applyQuery(params, data), function(obj) {
...
function applyQuery(query, items) {

    if (typeof query.where !== 'undefined') {

        items = _.filter(items, function(item) {

            return _.every(query.where, function(value, attr) {

                if (typeof value === 'object') {

                    var applyOperator = function(value, op) {

                        switch (op) {
                            case 'not':
                                if (typeof value === 'object') {
                                    return !_.some(value, applyOperator);
                                } else {
                    ...
                    return _.every(value, applyOperator);
    ...
    if (typeof query.order !== 'undefined') {
        ...
        if (query.order === 'RAND()') {
            ...
            var order = query.order.split(/\s+/),
                orderField = order[0],
                orderDirection = order[1];

            items = sortBy(items, orderField, orderDirection);
        }
    }

    if (typeof query.skip !== 'undefined') {
        items = _.rest(items, query.skip);
    }

    if (typeof query.limit !== 'undefined') {
        items = _.first(items, query.limit);
    }
